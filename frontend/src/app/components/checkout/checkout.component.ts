...
import { MyFormService } from 'src/app/services/my-form.service';
...
export class CheckoutComponent implements OnInit {
  ...
  reviewCartDetails() {
    ...
    // subscribe to cartService property
  ...
  onSubmit() {
    ...
    this.checkoutService.placeOrder(purchase).subscribe({
        next: response => {
          alert(`Il tuo ordine è stato ricevuto.\nNumero di tracking: ${response.orderTrackingNumber}`);
        ...
        error: err => {
          alert(`C'è stato un errore, ci scusiamo per il disagio: ${err.message}`);
        }
      }
    );
  }
